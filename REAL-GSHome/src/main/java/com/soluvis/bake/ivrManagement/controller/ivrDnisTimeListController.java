...
import com.soluvis.bake.ivrManagement.service.ivrDnisTimeListService;
 ...
 */
@Controller
@RequestMapping(value = "/api/ivr/ivrDnisTimeList")
public class ivrDnisTimeListController extends commController{
	
	@Inject
	private ivrDnisTimeListService ivrDnisListService;
	...
	public @ResponseBody List<ivrDnisTimeList> DnisListSearch(HttpServletRequest request) throws Exception {
		Map<String, Object> map = new HashMap<String, Object>(); 
		List<ivrDnisTimeList> search = null;
		...
		{	
			...
			logger.info("ivrDnisListService.DnisListGet Query Start...");	
			search = ivrDnisListService.DnisListGet(map);
		}
		catch(Exception e)
		{
    ...
    public ApiResponse DnisListSave(@Valid @RequestBody List<ivrDnisTimeList> dtLst, HttpServletRequest request) throws Exception {	
		Map<String, Object> map = new HashMap<String, Object>();
			...
			for (ivrDnisTimeList dtl : dtLst)
			{
				...
				{		
					...
					if(searchSize == 0)
					{
						...
						logger.info("ivrDnisListService.DnisListIst Query Start...");
						sqlrst = ivrDnisListService.DnisListIst(map);
					}
					else
					{
						logger.info("ivrDnisListService.DnisListDelUdt Query Start...");
						sqlrst = ivrDnisListService.DnisListDelUdt(map);
					}					
				...
				else if(AXBootTypes.DataStatus.MODIFIED.equals(dtl.getDataStatus()))
				{
					...
					logger.info("ivrDnisListService.DnisListUdt Query Start...");
					sqlrst = ivrDnisListService.DnisListUdt(map);		
				...
				{
					logger.info("ivrDnisListService.DnisListDel Query Start...");
					sqlrst = ivrDnisListService.DnisListDel(map);
			...
			if(result == 0)
			{
				...
				{				
			       try
			       {
			    	   ...
			    	   System.out.println("근무시간관리Url-->" + htpcode + Urlsearch.get(i).getSvr_ip() + urlcode); 
		    		   logger.info("근무시간관리Url-->" + htpcode + Urlsearch.get(i).getSvr_ip() + urlcode);
			    	   ...
			    	   if(in != null)
			    	   {
				    	   ...
				    	   {
				    		   System.out.println("근무시간관리-->" + line + "===>result-" + rcode); 
				    		   logger.info("근무시간관리-->" + line + "===>result-" + rcode);
				    	   ...
				    	   System.out.println("근무시간관리rlt-->" + rlt); 
			    		   logger.info("근무시간관리rlt-->" + rlt);
			    		   
				    	   System.out.println("근무시간관리err-->" + err.toString()); 
			    		   logger.info("근무시간관리err-->" + err.toString()) ;
