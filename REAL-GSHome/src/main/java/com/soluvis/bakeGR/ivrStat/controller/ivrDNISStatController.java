...
/** 
 * @author yejinlee
 * @desc   ivr 대표번호 통계 리스트를 조회한다.
 */
@Controller
@RequestMapping(value = "/gr/api/ivr/ivrDNISStat")
public class ivrDNISStatController extends commController{
	
	@Inject
	private ivrDNISStatService ivrDNISStatService; 
	...
	/** 
	 * @desc ivr 대표번호 통계 목록을 조회한다
	 */
	@RequestMapping(value="/dnisStatListSearch", method = RequestMethod.GET, produces = APPLICATION_JSON)
	public @ResponseBody List<Map<String, Object>> DNISStatListSearch(HttpServletRequest request) throws Exception {
		...
		{	
			...
			logger.info("ivrDNISStatService.DNISStatGet Query Start...");			
			search = ivrDNISStatService.DNISStatGet(map);
