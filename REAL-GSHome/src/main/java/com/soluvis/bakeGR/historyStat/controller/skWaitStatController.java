...
import com.soluvis.bakeGR.historyStat.service.skWaitStatService;
 ...
 */
@Controller
@RequestMapping(value = "/gr/api/hist/skWait")
public class skWaitStatController extends commController{
	
	@Inject
	private skWaitStatService skWaitStatService;	
	 ...
	 */ 
	@RequestMapping(value="/skWaitSearch", method = RequestMethod.GET, produces = APPLICATION_JSON)
	public @ResponseBody List<Map<String,Object>> skWaitSearch(@Valid @SQLInjectionSafe HttpServletRequest request) throws Exception {
		...
		if(Usearchsize > 0)
		{
			...
			// 조회
			search = skWaitStatService.skWaitSel(map);	
			...
			List<Map<String,Object>> searchsum = skWaitStatService.skWaitSelSum(map);			
