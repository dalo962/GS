...
import com.soluvis.bakeGR.historyStat.service.agSkillStatService;
...
/** 
 * @author gihyunpark
 * @desc   상담사 스킬 통계를 조회한다
 */
@Controller
@RequestMapping(value = "/gr/api/hist/agSkill")
public class agSkillStatController extends commController{
	
	@Inject
	private agSkillStatService agSkillStatService;	
	 ...
	 */ 
	@RequestMapping(value="/agSkillSearch", method = RequestMethod.GET, produces = APPLICATION_JSON)
	public @ResponseBody List<Map<String,Object>> agSkillSearch(@Valid @SQLInjectionSafe HttpServletRequest request) throws Exception {
		...
		if(Usearchsize > 0)
		{
			...
			// 조회
			search = agSkillStatService.agSkillSel(map);	
			...
			List<Map<String,Object>> searchsum = agSkillStatService.agSkillSelSum(map);			
