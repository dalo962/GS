...
import com.soluvis.bakeGR.historyStat.service.agProdtStatService;
...
/** 
 * @author gihyunpark
 * @desc   상담사 생산성 통계를 조회한다
 */
@Controller
@RequestMapping(value = "/gr/api/hist/agProdt")
public class agProdtStatController extends commController{
	
	@Inject
	private agProdtStatService agProdtStatService;	
	 ...
	 */ 
	@RequestMapping(value="/agProdtSearch", method = RequestMethod.GET, produces = APPLICATION_JSON)
	public @ResponseBody List<Map<String,Object>> agProdtSearch(@Valid @SQLInjectionSafe HttpServletRequest request) throws Exception {
		...
		if(Usearchsize > 0)
		{
			...
			// 조회
			search = agProdtStatService.agProdtSel(map);	
			...
			List<Map<String,Object>> searchsum = agProdtStatService.agProdtSelSum(map);			
