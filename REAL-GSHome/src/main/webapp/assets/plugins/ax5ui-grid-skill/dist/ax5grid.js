...
(function () {
    ...
    UI.addClass({
        ...
        return function () {
            ...
            /** 2020 콜통계 개선. xlsx 저장
             * jsp에서 xlsx.core.min.js , FileSaver.min.js, html2canvas.min.js 추가 해야됨.
             * @method ax5grid.exportExcelxlsx
             * @param {String} _fileName
             * @returns {ax5grid|String}
             * @example
             * ```js
             * firstGrid.exportExcelxlsx("grid-to-excel.xlsx");
             * console.log(firstGrid.exportExcel());
             * ```
             */
            this.exportExcelXlsx = function (_fileName) {
            	var table = [];                
                
                table.push(GRID.header.getExcelStringXlsx.call(this));
                table.push(GRID.body.getExcelStringXlsx.call(this)); 
                
                if (typeof _fileName === "undefined") {
                    return table.join('');
                } else {
                	GRID.excel.exportXlsx.call(this, table, _fileName, false);                	 
                }

                return this;
            };
            
            /** 2020 콜통계 개선. table 받아서 xlsx 저장. 
             * jsp에서 xlsx.core.min.js , FileSaver.min.js, html2canvas.min.js 추가 해야됨.
             * @method ax5grid.exportExcelxlsx
             * @param {String} _fileName
             * @returns {ax5grid|String}
             ...
             * ```
             */
            this.exportExcelXlsxTable = function (_fileName, table) {

                if (typeof _fileName === "undefined") {
                    return table.join('');
                } else {
                	GRID.excel.exportXlsx.call(this, table, _fileName, true); 
                }
...
(function () {
    ...
    var updateRowState = function updateRowState(_states, _dindex, _doindex, _data) {
        var self = this,
            ...
            "selected": function selected(_dindex, _doindex) {
                if (this.list[_doindex]) {
                    ...
                    while (i--) {
                    	if(false)
                    	{
                        this.$.panel[this.$.livePanelKeys[i]].find('[data-ax5grid-tr-data-index="' + _doindex + '"]').attr("data-ax5grid-selected", this.list[_doindex][cfg.columnKeys.selected]);
                    	}
                    }
                }
            },
            ...
            "cellChecked": function cellChecked(_dindex, _doindex, _data) {
                ...
                this.$.panel[panelName].find('[data-ax5grid-tr-data-index="' + _doindex + '"]').find('[data-ax5grid-column-rowIndex="' + rowIndex + '"][data-ax5grid-column-colIndex="' + colIndex + '"]').find('[data-ax5grid-editor="checkbox"]').attr("data-ax5grid-checked", '' + _data.checked);
            }
        };
    ...
    var updateRowStateAll = function updateRowStateAll(_states, _data) {
        var self = this,
            cfg = this.config,
            processor = {
            "selected": function selected(_doindex) {
                GRID.body.repaint.call(this, true);
            }
        };
            ...
            var panelName = void 0,
                ...
                "default": function _default(_column) {
                    ...
                    var column = self.bodyRowMap[_column.rowIndex + "_" + _column.colIndex],
                        value = "";
                    if (column) {
                        if (!self.list[doindex].__isGrouping) {
                    ...
                    if (U.isObject(editor)) {
                        ...
                        if (self.config.body.onDBLClick) {
                            var that = {
                                ...
                                page: self.page,
                                list: self.list,
                                //item: self.list[_column._doindex],
								item: self.list[_column.doindex],
                                dindex: _column.dindex,
                                doindex: _column.doindex,
                                rowIndex: _column.rowIndex,
                                colIndex: _column.colIndex,
                                column: column,
                                //value: self.list[_column._doindex][column.key]
								value: self.list[_column.doindex][column.key]
                            };
  ...
  //2020 콜통계 개선. xlsx 용으로 formatter에 있는 style태그들 제외하고 그냥 value만 추출 
    var getFieldValueDefault = function getFieldValue(_list, _item, _index, _col, _value, _returnPlainText) {
        ...
        if (_key === "__d-index__") {
            ...
            var valueProcessor = {       
            	"formatter": function formatter() {
                    ...
                    if (U.isFunction(_col.formatter)) {
                        //return _col.formatter.call(that);
                    	data = _col.formatter.call(that);                    	                    	
                    	
                    } else {
                        //return GRID.formatter[_col.formatter].call(that);
                    	data = GRID.formatter[_col.formatter].call(that);                    	
                    }
                    if(typeof data !== "undefined" && data !== null)	data = data.toString();
                    else	data = "";
                    if(typeof data !== "undefined" && data.indexOf('style') > 0){  //ax5grid.js 에서 사용
            			//<span style="font-weight:bold;color:#FF0000;">0.0</span>
            			//<div style="font-weight:bold;color:#FF0000;">0.0</div>
            			  var tmpData = data.substr(0, data.indexOf('</')); //ax5grid.js 에서 사용
            			  //var tmpData = tmp[i].substr(0, tmp[i].indexOf('</')); //plugins.js 에서 사용
            			  data = tmpData.substr(tmpData.indexOf('>')+1, tmpData.length-tmpData.indexOf('>')); //ax5grid.js 에서 사용 //<span style="font-weight:bold;color:#FF0000;">0.0
            			  //data = tmpData.substr(tmpData.lastIndexOf('>')+1, tmpData.length-tmpData.lastIndexOf('>')); //plugins.js 에서 사용 //<span style="font-weight:bold;color:#FF0000;">0.0		  
            		}
            ...
            var returnValue = _col.formatter ? valueProcessor.formatter.call(this) : valueProcessor.default.call(this);//valueProcessor.default.call(this); //2020 콜통계 개선
            /* tree 일때 사용하지만 xlsx에서는 값만 추출.
            if (this.config.tree.use && _col.treeControl) {
                returnValue = valueProcessor.treeControl.call(this, returnValue);
            }
			*/
  ...
  //2020 콜통계 개선. 
    //footsum 에서도 태그 없이 순수 value만 추출
    var getSumFieldValueDefault = function getSumFieldValueDefault(_list, _col) {
        var _key = _col.key,
            _label = _col.label;
        //, _collector, _formatter
        if (typeof _key === "undefined") {
            ...
            if (_col.collector) {
                ...
                if (_col.formatter) {
                    ...
                    if(typeof data !== "undefined" && data.indexOf('<') >= 0){ //<div><B><u>0</u></B></div>
      				  var bEnd = false;
      				  while(!bEnd){
      					  var tag = data.substr(data.indexOf('<')+1,data.indexOf('>')-1); //div
      					  
      					  data = data.substr(tag.length+2, data.lastIndexOf('</')-(tag.length+2));
    ...
    var repaintCell = function repaintCell(_panelName, _dindex, _doindex, _rowIndex, _colIndex, _newValue) {
        ...
        updateCell.html(getFieldValue.call(this, list, list[_doindex], _dindex, col));
            ...
            col.editor.updateWith.forEach(function (updateColumnKey) {
                colGroup.forEach(function (col) {
                    if (col.key == updateColumnKey) {
                        ...
                        updateWithCell.html(getFieldValue.call(self, list, list[_doindex], _dindex, col));
                    }
                });
            });
        }
    ...
    var repaintRow = function repaintRow(_dindex) {
        ...
        // left
        if (this.xvar.frozenColumnIndex > 0) {
            if (this.xvar.frozenRowIndex > _doindex) {
                // 상단 행고정
                replaceTr.call(this, "top-left-body", this.leftHeaderColGroup, leftBodyRowData, list.slice(0, this.xvar.frozenRowIndex), _doindex);
            } else {
                replaceTr.call(this, "left-body-scroll", this.leftHeaderColGroup, leftBodyRowData, list, _doindex);
            }
        }
        ...
        if (this.xvar.frozenRowIndex > _doindex) {
            // 상단 행고정
            replaceTr.call(this, "top-body-scroll", this.headerColGroup, bodyRowData, list.slice(0, this.xvar.frozenRowIndex), _doindex);
        } else {
            replaceTr.call(this, "body-scroll", this.headerColGroup, bodyRowData, list, _doindex);
        }
  ...
  //2020 콜통계 개선. xlsx export용
    //tr, td 태그 없이 grid body의 순수 data, colspan, rowspan 값만 추출
    var getExcelStringXlsx = function getExcelStringXlsx() {
        var cfg = this.config,
            list = this.list,
            bodyRowData = this.bodyRowTable,
            footSumData = this.footSumTable,
            ...
            for (di = 0, dl = _list.length; di < dl; di++) {
                ...
                for (tri = 0, trl = rowTable.rows.length; tri < trl; tri++) {  
                    ...
                    for (ci = 0, cl = rowTable.rows[tri].cols.length; ci < cl; ci++) {
                        ...
                        if(col.width != 0)                        	
                        	CD.push({colspan:col.colspan, rowspan:col.rowspan, data: isGroupingRow ? getGroupingValue.call(this, _list[di], di, col) : getFieldValueDefault.call(this, _list, _list[di], di, col, val, "text") }) //formatter 안 타게 getFieldValueDefault 호출
                    }
                    RD.push(CD);
                }
            }
        ...
        var getSum = function getSum(_colGroup, _bodyRow, _list) {
            ...
            for (tri = 0, trl = _bodyRow.rows.length; tri < trl; tri++) {
                ...
                for (ci = 0, cl = _bodyRow.rows[tri].cols.length; ci < cl; ci++) {
                    ...
                    if(ci == 0)
                    	//CD.push({data: getSumFieldValue.call(this, _list, col), colspan:col.colspan - hidden, rowspan:col.rowspan });
                    	CD.push({data: getSumFieldValueDefault.call(this, _list, col), colspan:col.colspan - hidden, rowspan:col.rowspan }); //footsum 에서도 태그 없이 순수 value만 추출
                    else
                    	//CD.push({data: getSumFieldValue.call(this, _list, col), colspan: col.colspan, rowspan:col.rowspan });
                    	CD.push({data: getSumFieldValueDefault.call(this, _list, col), colspan: col.colspan, rowspan:col.rowspan }); //footsum 에서도 태그 없이 순수 value만 추출
                }
                RD.push(CD);
            }
    ...
    GRID.body = {
        ...
        getExcelStringXlsx: getExcelStringXlsx,
...
(function () {
    ...
    var setValue = function setValue(_dindex, _doindex, _key, _value) {
        ...
        if (originalValue !== _value) {
            if (/[\.\[\]]/.test(_key)) {
                try {
                    this.list[_doindex][this.config.columnKeys.modified] = true;
                    Function("val", "this" + GRID.util.getRealPathForDataItem(_key) + " = val;").call(this.list[_doindex], _value);
                } catch (e) {}
            } else {
                this.list[_doindex][this.config.columnKeys.modified] = true;
                this.list[_doindex][_key] = _value;
            }
...
(function () {
	var defaults = {			
			...
			mso: {                            // MS Excel and MS Word related options
		        ...
		        styles:            ['background-color','font-weight','text-align','width','font-size'],          // E.g. ['border-bottom', 'border-top', 'border-left', 'border-right']
		        worksheetName:     '',
    ...
    //var uri = "data:application/vnd.openxmlformets-officedocument.spreadsheetml.sheet;base64,";
    ...
    //2020 콜통계 추가. xlsx 저장용.
    var tableToExcelXlsx = function tableToExcelXlsx(table, fileName, bHtml) {
        var sheetNames = [];
        var workbook = XLSX.utils.book_new();
    	
        var ws = ({});
        
        if(!bHtml)	ws = xlsxTableToSheet(table); //ax5ui grid용 (tr,td 등 태그 없음)
        else	ws = xlsxTableToSheetTable(table); //html tag 사용한 그리드용 (sub화면에서 사용)
        
        var sheetName = '';
        sheetName = fileName.trim(fileName.replace(/[\\\/[\]*:?'"]/g, '').substring(0, 31));
        sheetNames.push(sheetName);
        
        XLSX.utils.book_append_sheet(workbook, ws, sheetName);
        
        // add worksheet to workbook
        try{ //2020 콜통계 개선. try catch 추가
  	      ...
  	      saveToFile(xlsxWorkbookToArrayBuffer(wbout),
  	        fileName + '.' + defaults.mso.fileFormat,
    ...
    var xlsxWorkbookToArrayBuffer = function xlsxWorkbookToArrayBuffer (s) {
    ...
    var xlsxTableToSheet = function xlsxTableToSheet (table) {
    	var ws = ({});
 
        if(table.length > 0){
	        var rows = parsingRows(table);
	        ...
	        for (; _R < rows.length && R < sheetRows; ++_R) {
	          ...
	          _CLength = 0;
	          for (_C = 0; _C < row.length; ++_C) {
	            elt = row[_C];
	            CS = parseInt(elt.colspan);
	            _CLength += CS;
	          }
	          ...
	          for (_C = C = 0; _C < row.length; ++_C) {
	            ...
	            for (midx = 0; midx < merges.length; ++midx) {
	              if (merges[midx].s.c == C && merges[midx].s.r <= R && R <= merges[midx].e.r) {
	                C = merges[midx].e.c + 1;
	                midx = -1;
	              }
	            }
	
	            if ((RS = parseInt(elt.rowspan)) > 0 || CS > 1)
	              merges.push({s: {r: R, c: C}, e: {r: R + (RS || 1) - 1, c: C + CS - 1}}); //merge 되어야 하는 cell들
	            
	            var o = {t: 's', v:  elt.data};
    ...
    //html 테이블 용
      var xlsxTableToSheetTable = function xlsxTableToSheet (table) {
          var ws = ({});
          
          if(table.length > 0){
  	        var rows = parsingRowsTable(table[0]);
  	        var sheetRows = 10000000;
  	        var range = {s: {r: 0, c: 0}, e: {r: 0, c: 0}};
  	        var merges = [], midx = 0;
  	        var rowinfo = [];
  	        ...
  	        for (; _R < rows.length && R < sheetRows; ++_R) {
  	          ...
  	          if(row == "") continue; //tr 밑에 td 없으면 무시.
  	 
  	          var elts = parsingColumns(row); //row 안의 data, colspan, rowspan 파싱
      ...
      var parsingRows = function parsingRows (table) {
    	  var tmpRows = [];
    	  
    	  //header
    	  for(var i=0; i<table[0].length; i++){	  
    		  tmpRows.push(table[0][i]);
    	  }
    	  
    	  //body
    	  for(var j=0; j<table[1].length; j++){
    		  for(var k=0; k<table[1][j].length; k++){
    			  tmpRows.push(table[1][j][k]);
    		  }
    	  }
    	  
    	  return tmpRows;   	  
      };
      var parsingRowsTable = function parsingRowsTable (table) {
    	  var tmp = table.replace('<table border=\"1\">','');
    	  tmp = tmp.replace('</table>','');
    	  
    	  var tmpRows = tmp.split('<tr>');
    	  
    	  for(var i=0; i<tmpRows.length; i++){    		  
    		  tmpRows[i] = tmpRows[i].replace('</tr>','');  

    		  if(tmpRows[i].substr(tmpRows[i].length-2,2) === "\\n"){
    			  tmpRows[i] = tmpRows[i].substr(0,tmpRows[i].length-2);
    		  }    		  
    	  }
      ...
      var parsingColumns = function parsingColumns (row) {
    	  // row
    	  //td 태그   <td colspan=\"1\" rowspan=\"1\" >계약상담</td><td colspan=\"1\" rowspan=\"1\" >0</td>
    	  //html에서 그렸으면 th 태그   <th width='70px' align='center' style=\"mso-number-format:'@';\">2</th><th width='150px' align='center'>자동차 고장 긴급출동</th><th width='200px' align='center'></th><th width='200px' align='center'></th><th width='200px' align='center'></th><th width='200px' align='center'></th><th width='200px' align='center'></th>
    	  var tmpColumns = [];
    	  var tmp = []; //row.split('</td>'); //tmp[0] : <td colspan=\"1\" rowspan=\"1\" >계약상담, tmp[1] : <td colspan=\"1\" rowspan=\"1\" >0
    	  
    	  if(row.indexOf('td') >= 0)
    		  tmp = row.split('</td>');
    	  else if(row.indexOf('th') >= 0)
    		  tmp = row.split('</th>');
    	  ...
    	  for(var i=0; i<tmp.length; i++){
    		  ...
    		  tmp[i] = tmp[i].replace(/\\/gi,'');//tmp[0] : <td colspan="1" rowspan="1" >계약상담, tmp[1] : <td colspan="1" rowspan="1" >0
    		  
    		  //var data = tmp[i].substr(tmp[i].indexOf('>')+1, tmp[i].length-tmp[i].indexOf('>') );  //plugins.js 에서 사용
    		  var data = tmp[i].substr(tmp[i].lastIndexOf('>')+1, tmp[i].length - tmp[i].lastIndexOf('>'));  //ax5grid.js 에서 사용
    		  
    		  if(data.indexOf('style') > 0){  //ax5grid.js 에서 사용
    			//<span style="font-weight:bold;color:#FF0000;">0.0</span>
    			//<div style="font-weight:bold;color:#FF0000;">0.0</div>
    			  if(data.indexOf('</') > 0){
	    			  var tmpData = data.substr(0, data.indexOf('</')); //ax5grid.js 에서 사용
			  ...
			  if(data.indexOf('<') >= 0){ //<div><B><u>0</u></B></div>
				  var bEnd = false;
				  while(!bEnd){
					  var tag = data.substr(data.indexOf('<')+1,data.indexOf('>')-1); //div
			  ...
			  if(data.indexOf('\&nbsp\;') >= 0)  data = data.replace(/\&nbsp\;/gi,'');
			  
			  
    		  //colspan 정보    		   
    		  var colspan =  tmp[i].substr(tmp[i].indexOf('colspan'), tmp[i].indexOf('rowspan') - tmp[i].indexOf('colspan')-1); //colspan="1"
    		  colspan = colspan.replace(/"/gi,"");
    		  colspan = colspan.substr(8,colspan.length-8).replace(/\\/gi,"");
    		  
    		  //rowspan 정보
    		  var rowspan =  tmp[i].substr(tmp[i].indexOf('rowspan'), tmp[i].indexOf('>') - tmp[i].indexOf('rowspan')-1); //rowspan="1"
    		  rowspan = rowspan.replace(/"/gi,"");
    		  rowspan = rowspan.substr(8,rowspan.length-8).replace(/\\/gi,"");
    		  
    		  
    		  //parseString 부분 cellInfo.cellType 필요 없을듯?
    		  var cellType = 'text'; //text
    		  
    		  if(isFinite(xlsxToNumber(data, defaults.numbers.output)))
    			  cellType = 'number'; //number
    		  
    		  tmpColumns.push({data:data, colspan:colspan, rowspan:rowspan, cellType:cellType});
    	  }    	     	  
    	  
    	  return tmpColumns;    	  
     };
     
     var xlsxToNumber = function xlsxToNumber (s, numbersFormat) {
         var v = Number(s);
    ...
    var parseDateUTC = function parseDateUTC (s) {
    ...
    var xlsxEncodeRow = function xlsxEncodeRow (row) { return '' + (row + 1); };

    var xlsxEncodeCol = function xlsxEncodeCol (col) {
        var s = '';
        for (++col; col; col = Math.floor((col - 1) / 26)) s = String.fromCharCode(((col - 1) % 26) + 65) + s;
        return s;
    };    
    var xlsxEncodeCell = function xlsxEncodeCell (cell) { return xlsxEncodeCol(cell.c) + xlsxEncodeRow(cell.r); };
    
    var xlsxEncodeRange = function xlsxEncodeRange (cs, ce) {
     ...
     var saveToFile =  function saveToFile (data, fileName, type, charset, encoding, bom) {
         var saveIt = true;
     ...
     var downloadFile =  function downloadFile (filename, header, data) {
...
(function () {
    ...
    var select = function select(_options) {
        GRID.data.select.call(this, doindex, _options && _options.selected);
        GRID.body.updateRowState.call(this, ["selected"], doindex);
    };
    ...
    //2020 콜통계 개선. xlsx export용
    //tr, td 없이 header의 cell별 정보 추출 (data string, colspan, rowspan) 
    var getExcelStringXlsx = function getExcelStringXlsx() {
        var cfg = this.config,
            colGroup = this.colGroup,
            headerData = this.headerTable,
            getHeader = function getHeader(_colGroup, _bodyRow) {
	            ...
	            for (var tri = 0, trl = _bodyRow.rows.length; tri < trl; tri++) {
	                ...
	                for (var ci = 0, cl = _bodyRow.rows[tri].cols.length; ci < cl; ci++) {
	                    ...
	                    if(col.width != 0)	                    	
	                    	CD.push({data:getFieldValue.call(this, col), colspan:col.colspan, rowspan:col.rowspan});
	                }
	                RD.push(CD);
	            }
        ...
        "click": function click(track, bar, type, e) {
            ...
            var self = this,
                ...
                "vertical": function vertical(e) {
                    ...
                    return { top: newTop };
                    */
                	// 20190925 마우스클릭값을 받아서 현재 스크롤의 위치와 비교한다
                	var mouseObj = GRID.util.getMousePosition(e); 
                	var focus = mouseObj.clientY - trackOffset.top;
                	var newTop;
                	
                	var verticalScroll = self.$["scroller"]["vertical-bar"][0].offsetTop;
                    var horizontalScroll = self.$["scroller"]["horizontal-bar"][0].offsetLeft;
                	
                    var scrollInter = null;
                    var scrollclicker = null;
                    
                    var width_scrollInter = null;
                    var width_scrollclicker = null;
                    
                    /**2020.10.22 스크롤 조정**/
                    scrollInter = _panel_height/(_content_height/_panel_height);
                    scrollclicker = _panel_height/(_content_height/28);
                    width_scrollInter = _panel_width/(_content_width/_panel_width);
                    width_scrollclicker = _panel_width/(_content_width/80);
                    
                	//console.log("mouseObj->" + mouseObj);
                	//console.log("focus->" + focus);
                	//console.log("newTop->" + newTop);
                	 
                	// 스크롤에서 현재 자리를 누를 경우 현재 자리를 리턴 (위아래 좌우 형식모두 동일함)
                	if(focus == verticalScroll)
                	{
                		newTop = verticalScroll
                		//console.log("1verticalScroll->" + verticalScroll);
                	}
                	else
                	{
                		// 클릭한 자리가 현재 자리보다 좌표가 높은 경우 스크롤 아래로 이동
                		if(focus > verticalScroll)
                		{
                			// 클릭한 자리가 스크롤의 총 넓이 - 15 보다 높을 경우 가변적인 값이므로 위 변수 사용  거의 아래 끝자리를 클릭한 경우이다
                			if(focus >= (_vertical_scroller_height - 15))
                			{
                				// 간격만큼 스크롤을 이동시킴
                				if(scrollclicker<1) scrollclicker = 1;
                				newTop = verticalScroll + scrollclicker
                    			//console.log("2verticalScroll->" + verticalScroll);
                			...
                			{
                				//newTop = focus;
                				newTop = verticalScroll + scrollInter
                				//console.log("newTop->" + newTop);
                			}
                		}
                		// 클릭한 자리가 현재 자리보다 좌표가 낮은 경우 스크롤 위로 이동
                		else
                		{
                			// 클릭한 자리가 스크롤의 총 넓이 - 15 보다 낮을 경우 가변적인 값이므로 위 변수 사용  거의 위 끝자리를 클릭한 경우이다
                			if(focus <= 15)
                			{
                				// 간격만큼 스크롤을 이동시킴
                				if(scrollclicker<1) scrollclicker = 1;
                				newTop = verticalScroll - scrollclicker
                    			//console.log("3verticalScroll->" + verticalScroll);
                    			//console.log("scrollInter->" + scrollInter);
                    			//console.log("newTop->" + newTop);
                			}
                			else // 스크롤 이동시킴
                ...
                "horizontal": function horizontal(e) {
                    ...
                    */
                	var mouseObj = GRID.util.getMousePosition(e);
                	var focus = mouseObj.clientX - trackOffset.left;
                	var newLeft;
                	...
                	if(focus == horizontalScroll)
                	{
                		newLeft = horizontalScroll;
                		//console.log("1horizontalScroll->" + horizontalScroll);
                	}
                	else
                	{
                		if(focus > horizontalScroll)
                		{
                			if(focus >= (_horizontal_scroller_width - 15))
                			{
                				newLeft = horizontalScroll + width_scrollclicker
	                			//console.log("2horizontalScroll->" + horizontalScroll);
	                			//console.log("scrollInter->" + scrollInter);
	                			//console.log("newLeft->" + newLeft);
                			}
                			else
                			{
                				newLeft = horizontalScroll + width_scrollInter
                				//newLeft = focus;
                				//console.log("newLeft->" + newLeft);
                			}
                		}
                		else
                		{
                			...
                			{
	                			newLeft = horizontalScroll - width_scrollclicker
	                			//console.log("3horizontalScroll->" + horizontalScroll);
	                			//console.log("scrollInter->" + scrollInter);
	                			//console.log("newLeft->" + newLeft);
                			}
                			else
