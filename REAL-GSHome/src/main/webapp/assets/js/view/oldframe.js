...
fnObj.pageStart = function () {
    ...
    axboot.ajax({
        ...
        callback: function (res) {
            ...
            top.wsConn(sockInfo['STAT_HOSTP'], sockInfo['STAT_PORTP'], sockInfo['STAT_HOSTB'], sockInfo['STAT_PORTB'], sockInfo['STAT_WSS']) ;
        }
    });
...
fnObj.tabView = axboot.viewExtend({
            ...
            switch (this.action) {
                case "reload":
                    fnObj.tabView.list.forEach(function (_item, idx) {
                        if (_item.status == "on") {
                            //window["frame-item-" + _item.menuNm].location.reload();
                        	//window["frame-item-" + idx].location.reload();
                ...
                case "close":
                    fnObj.tabView.list.forEach(function (_item, idx) {
                        if (_item.status == "on") {
                            ...
                            //fnObj.tabView.close(_item.menuNm);
    ...
    _getItem: function (item) {
        var po = [];
        //po.push('<div class="tab-item ' + item.status + '" data-tab-id="' + item.menuNm + '">');
        ...
        po.push('<span data-toggle="tooltip" data-placement="bottom" title=\'' + item.menuNm + '\'>', item.menuNm, '</span>');
        //if (!item.fixed) po.push('<i class="cqc-cancel3" data-tab-close="true" data-tab-id="' + item.menuNm + '"></i>');
    ...
    _getFrame: function (item) {
        var po = [];
        //po.push('<iframe class="frame-item ' + item.status + '" data-tab-id="' + item.menuNm + '" name="frame-item-' + item.menuId + '" src="' + item.url + '" frameborder="0" framespacing="0" allowFullScreen="true"></iframe>');
    ...
    open: function (item) {
        ...
        var findedIndex = ax5.util.search(this.list, function () {
            this.status = '';
            //return this.menuNm == item.menuNm;
        ...
        if (findedIndex < 2) {        		
            ...
            //this.target.find('[data-tab-id="' + _item.menuNm + '"]').addClass("on");
            this.target.find('[data-tab-id="' + _item.menuId + '"]').addClass("on");
            //this.frameTarget.find('[data-tab-id="' + _item.menuNm + '"]').addClass("on");
    ...
    close: function (menuNm) {
        ...
        this.list = newList;
        //this.target.find('[data-tab-id="' + menuNm + '"]').remove();
        this.target.find('[data-tab-id="' + menuId + '"]').remove();

        // 프레임 제거
        (function () {
            //var $iframe = this.frameTarget.find('[data-tab-id="' + menuNm + '"]'), // iframe jQuery Object
        ...
        if (removeItem.status == 'on') {
            ...
            //this.target.find('[data-tab-id="' + this.list[lastIndex].menuNm + '"]').addClass("on");
            this.target.find('[data-tab-id="' + this.list[lastIndex].menuId + '"]').addClass("on");
            //this.frameTarget.find('[data-tab-id="' + this.list[lastIndex].menuNm + '"]').addClass("on");
