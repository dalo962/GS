...
var ACTIONS = axboot.actionExtend(fnObj, {
    ...
    PAGE_SAVE: function (caller, act, data) {
        ...
        if(typeof formData.menuId !== "undefined")
        {
        	axboot.ajax({
        		type: "POST",
 	            	 ...
 	            	 if (data && data.callback) {
 	                    ...
 	                    if (formData.progCd) {                    	
 	                        axboot.ajax({
 	                            ...
 	                            callback: function (res) {
 	                                //axToast.push(LANG("ax.script.menu.authgroup.saved")); 주석처리
 	                                //ACTIONS.dispatch(ACTIONS.SEARCH_AUTH, {menuId: caller.formView01.getData().menuId}); 주석처리
 	                            	ACTIONS.dispatch(ACTIONS.M_PAGE_SAVE);
 	                            }
 	                        });
 	                    } else {
    ...
    M_PAGE_SAVE: function (caller, act, data) {
    	var formData = caller.formView01.getData();
    	// 메뉴에 대한 권한 수정시 반영해주며 auth_group_m과 똑같은 프로세스 적용 삭제한 menu_id를 쓰레기값이 된다
    	if (formData.progCd) {
	        axboot.ajax({
	            type: "POST",
	            url: "/api/menuMng/menuMngGrpSave",
	            data: JSON.stringify(caller.gridView01.getData()),
	            callback: function (res) {
...
fnObj.gridView01 = axboot.viewExtend(axboot.gridView, {
    initView: function () {
        ...
        this.target = axboot.gridBuilder({
            ...
            columns: [
                //menuId
                {key: "grpAuthCd", label: COL("ax.admin.menu.auth.group.code"), width: 100, align: "center"},
