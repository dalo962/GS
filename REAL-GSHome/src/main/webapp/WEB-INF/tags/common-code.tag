...
<%@ tag import="com.soluvis.bake.system.domain.code.CommonCode" %>
...
<%
    ...
    switch (type) {
        ...
        case "radio":
            for (CommonCode commonCode : commonCodes) {
                if (StringUtils.isNotEmpty(defaultValue) && defaultValue.equals(commonCode.getCode())) {
                    builder.append(String.format("<label class=\"checkbox-inline\"><input type=\"radio\" id=\"%s\" name=\"%s\" data-ax-path=\"%s\" value=\"%s\" value2=\"%s\" checked> %s </label>", commonCode.getSort(), name, dataPath, commonCode.getCode(), commonCode.getSort(), commonCode.getName()));
                } else {
                    builder.append(String.format("<label class=\"checkbox-inline\"><input type=\"radio\" id=\"%s\" name=\"%s\" data-ax-path=\"%s\" value=\"%s\" value2=\"%s\"> %s </label>", commonCode.getSort(), name, dataPath, commonCode.getCode(), commonCode.getSort(), commonCode.getName()));
                }
            }
            break;
    }
