...
<div data-page-buttons="">
    <div class="button-warp">
        ...
        <c:if test="${authGroupMenu.fn1Ah eq 'Y'}">
            <button type="button" class="btn btn-fn1" data-page-btn="fn1">항목${function1Label}</button>
        </c:if>

        <c:if test="${authGroupMenu.fn2Ah eq 'Y'}">
            <button type="button" class="btn btn-fn1" data-page-btn="fn2">통계항목${function2Label}</button>
        </c:if>

        <c:if test="${authGroupMenu.fn3Ah eq 'Y'}">
            <button type="button" class="btn btn-fn1" data-page-btn="fn3">설정${function3Label}</button>
        </c:if>

        <c:if test="${authGroupMenu.fn4Ah eq 'Y'}">
            <button type="button" class="btn btn-fn1" data-page-btn="fn4">모니터링${function4Label}</button>
        </c:if>

        <c:if test="${authGroupMenu.fn5Ah eq 'Y'}">
            <button type="button" class="btn btn-fn1" data-page-btn="fn5">로그아웃${function5Label}</button>
        </c:if>
